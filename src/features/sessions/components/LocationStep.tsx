...
export function LocationStep({ city }: LocationStepProps) {
    ...
    <div className="flex flex-col items-center">
      <div className="text-center mb-10">
        <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl text-charcoal mb-3">
          {locale === 'ar' ? 'المواقع المقترحة' : 'Suggested Locations'}
        </h2>
        <p className="font-sans text-xs tracking-[0.18em] uppercase text-charcoal/50">
          {locale === 'ar' ? 'أماكن التصوير المعتمدة للاستوديو' : 'Our studio curated photography venues'}
        </p>
      </div>
        ...
        {locations.map((loc, i) => {
          ...
          return (
            <motion.div
              ...
              className="text-left rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 relative border border-charcoal/10 bg-white shadow-xs"
            >
              <div>
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-charcoal/05 text-charcoal/60">
        ...
        <p className="font-sans text-xs sm:text-sm text-charcoal/75 leading-relaxed text-left">
          {locale === 'ar' ? (
            <>
              ...
              المواقع المدرجة هي مواقعنا الموصى بها، ويتم تحديد وتنسيق موقع التصوير النهائي بالاتفاق المباشر مع المصور.
            </>
          ) : (
            <>
              <span className="font-semibold text-charcoal">Note:</span>{' '}
              The listed locations are our recommended venues. The final photography location will be arranged and confirmed directly with the photographer.
            </>
          )}
        </p>
      </div>
